...
<head>
    ...
    <title>Lupa Password • RiauPort</title>
    ...
    <div class="w-full max-w-[950px] bg-white shadow-[0_2px_10px_-3px_rgba(14,14,14,0.3)] rounded-2xl overflow-hidden">
        <div class="flex items-center max-md:flex-col w-full gap-y-4">
                ...
                <div
                    ...
                    <div class="absolute inset-0 flex items-center justify-center">
                        <div class="w-full text-center pt-56 px-6 max-md:hidden">
                            <h1 class="fell text-[var(--primary)] text-3xl font-bold mb-3">Lupa Password?</h1>
                            <p class="fell text-gray-700 text-[15px] font-medium leading-relaxed">
                                1) Masukkan email akunmu<br>
                                2) Kami kirim link reset password<br>
                                3) Buka email, lalu buat password baru
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="w-full h-full px-8 lg:px-16 py-8 max-md:-order-1">
                <form method="POST" action="{{ route('password.email') }}" class="md:max-w-md w-full mx-auto">
                    @csrf

                    <div class="mb-8">
                        <h3 class="text-4xl font-bold text-[var(--primary)]">Reset Password</h3>
                        <p class="text-gray-600 text-sm mt-2">Masukkan email yang terdaftar di RiauPort</p>
                    </div>

                    @if (session('status'))
                        <div
                            class="mb-6 p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-xs text-emerald-800 text-center">
                            {{ session('status') }}
                        </div>
                    @endif
